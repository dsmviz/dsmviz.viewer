...
{
    public class CoreDsmFile(string filename, IModelImport modelImport)
    {
        ...
        private readonly IMetaDataModelImport _metaDataModelImport = modelImport.MetaDataModelImport;
        private readonly IElementModelImport _elementModelImport = modelImport.ElementModelImport;
        private readonly IRelationModelImport _relationModelImport = modelImport.RelationModelImport;
        ...
        {
            modelImport.ModelVersion = modelImport.ModelVersion + 1;
            writer.WriteAttributeString(ModelVersionXmlAttribute, modelImport.ModelVersion.ToString());

            _totalElementCount = _elementModelImport.GetPersistedElementCount() - 1; // Root not written/read
            writer.WriteAttributeString(ModelElementCountXmlAttribute, _totalElementCount.ToString());
            _progressedElementCount = 0;

            _totalRelationCount = _relationModelImport.GetPersistedRelationCount();
        ...
        protected virtual void ReadModelAttributes(XmlReader xReader)
        {
            if (xReader.Name == RootXmlNode)
            {
                ...
                modelImport.ModelVersion = modelVersion ?? 0;
        ...
        {
            foreach (string group in _metaDataModelImport.GetMetaDataGroups())
            {
        ...
        private void WriteMetaDataGroup(XmlWriter writer, string group)
        {
            ...
            foreach (IMetaDataItem metaDataItem in _metaDataModelImport.GetMetaDataGroupItems(group))
            {
                ...
                if (group != null)
                {
                    ...
                    while (xMetaDataReader.Read())
                    {
                        if (xMetaDataReader.Name == MetaDataXmlNode)
                        {
                            ...
                            {
                                _metaDataModelImport.ImportMetaDataItem(group, name, value);
                            }
                        }
                    }
                }
        ...
        private void WriteElements(XmlWriter writer, IFileProgress progress)
        {
            ...
            foreach (IElement element in _elementModelImport.GetRootElement().PersistedChildren)
            {
        ...
        private void ReadElement(XmlReader xReader, IFileProgress progress)
        {
            if (xReader.Name == ElementXmlNode)
            {
                ...
                {
                    _elementModelImport.ImportElement(id.Value,
                        name,
                        type,
        ...
        private void WriteRelations(XmlWriter writer, IFileProgress progress)
        {
            ...
            foreach (IRelation relation in _relationModelImport.GetPersistedRelations())
            {
        ...
        private void ReadRelation(XmlReader xReader, IFileProgress progress)
        {
            if (xReader.Name == RelationXmlNode)
            {
                ...
                {
                    IElement? consumer = _elementModelImport.GetElementById(consumerId.Value);
                    IElement? provider = _elementModelImport.GetElementById(providerId.Value);

                    if (consumer != null && provider != null)
                    {
                        _relationModelImport.ImportRelation(id.Value, consumer, provider, type, weight.Value,
