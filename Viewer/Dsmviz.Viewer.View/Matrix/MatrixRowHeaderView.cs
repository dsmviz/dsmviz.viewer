    ...
    public class MatrixRowHeaderView : Canvas
    {
        ...
        private void OnDataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            if (DataContext is IMatrixViewModel matrixViewModel)
            {
                ...
                _matrixRowHeaderViewModel.ReloadRequested += OnReloadRequested;
                _matrixRowHeaderViewModel.RedrawRequested += OnRedrawRequested;
            }
        }
        ...
        private void CreateChildViews()
        {
            ...
            Children.Clear();
            if (_matrixRowHeaderViewModel != null)
            {
        ...
        private void CreateChildViews(IMatrixRowHeaderTreeItemViewModel elementViewModel, double y)
        {
            foreach (IMatrixRowHeaderTreeItemViewModel child in elementViewModel.Children)
            {
                ...
                if (_matrixRowHeaderViewModel != null)
                {
                    MatrixRowHeaderTreeItemView elementView =
                        new MatrixRowHeaderTreeItemView(_matrixRowHeaderViewModel, _theme)
                        {
                            Height = rect.Height,
                            Width = rect.Width
                        };
