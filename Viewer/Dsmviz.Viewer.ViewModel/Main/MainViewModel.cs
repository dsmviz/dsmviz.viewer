...
namespace Dsmviz.Viewer.ViewModel.Main
{
    public class MainViewModel : ViewModelBase, IMainViewModel
    {
        ...
        public MainViewModel(string appName, IEditing editing, IQuery query, IMatrix matrix, IMetrics metrics, IStorage storage)
        {
            _appName = appName;
            ...
            _storage = storage;
            _storage.Modified += OnModelModified;

            _progressViewModel = new ProgressViewModel();

            _matrixViewModel = new MatrixViewModel(this, matrix, metrics, _storage, editing, query);
        ...
        private void OnSearchUpdated(object? sender, EventArgs e)
        {
            ...
            MatrixViewModel.ContentChanged(ContentChangeType.Search);
        }
        ...
        public ViewPerspective SelectedViewPerspective
        {
            ...
            set
            {
                ...
                OnPropertyChanged();
                MatrixViewModel.ContentChanged(ContentChangeType.Perspective);
            }
        }
        ...
        private async void OpenFileExecute(object? parameter)
        {
            if (parameter is string filename)
            {
                    ...
                    case ".dsm":
                        {
                            ...
                            await _storage.AsyncOpenDsmModel(fileInfo, progress);
                    ...
                    case ".dsi":
                        {
                            ...
                            await _storage.AsyncImportModel(fileInfo, progress);
                ...
                _matrixViewModel.ContentChanged(ContentChangeType.ModelLoaded);
            }
        }
        ...
        private async void SaveFileExecute(object? parameter)
        {
            ...
            await _storage.AsyncSaveDsmModel(fileInfo, progress);
        }

        private bool SaveFileCanExecute(object? parameter)
        {
            return _storage.IsModified;
        }
        ...
        private void OnActionPerformed(object? sender, EventArgs e)
        {
            ...
            MatrixViewModel.ContentChanged(ContentChangeType.Action);
        ...
        {
            IncludeAllInTree();
            _matrixViewModel.ContentChanged(ContentChangeType.MatrixView);
        }
