...
namespace Dsmviz.Test.Data.Import.Dsi
{
    ...
    public class DsiModelFileTest
    {
        ...
        {
            string inputFilename = "Validation.Input.dsi";

            DsiModel dsiModel = new DsiModel(_dataModel, _dataModel);
            dsiModel.ImportModel(inputFilename, true, _fileProgress.Object);

            IDictionary<string, List<IMetaDataItem>> metaData = _dataModel.MetaDataModelImport.GetMetaDataItems();
            ...
            Element? rootElement = _dataModel.ElementModelImport.GetRootElement() as Element;
            ...
            List<IRelation> relations =
                _dataModel.RelationModelImport.GetPersistedRelations().OrderBy(r => r.Id).ToList();
